...
if USE_MANUAL_PARSE:
    txt_pages = open("raw_ocr.txt", "r").read()
else: 
    try:
...
# Monday, May 5, 2025
date_regex = r"(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday), (April|May|December) \d{1,2}, 2025"

instructor_regex = r"([A-Z]+ [A-Z]+ \([a-z]*\))(, [A-Z]+ [A-Z]+ \(.*\))*$"

full_line_regex = r"(\d{5} [A-Z0-9]{1,4}.*) *((Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday), (April|May|December) \d{1,2}, 2025) *(\d{2}:\d{2}[ap]m - \d{2}:\d{2}[ap]m) *((Remote|In Person) (To Be Scheduled after the Mini-2 Add Deadline|Remote|(PTC|AH|AN|BH|PH|BR|CIC|CUC|CFA|CYH|DH|FM|GHC|HOA|HBH|HH|HL|MM|MI|NSH|PC|POS|PCA|REH|SC|EDS|TCS|TEP|WH|WEH|WWG|WF|WQ|2SC|3SC|4SC|CC|UT|FRB|INI|PO|MC|BOS|CLY|DON|FAF|FCL|FIF|FBA|GQ1|GQ2|GQ3|GQ4|GQ5|GQ6|HAM|HEN|HIL|MMA|MCG|MOE|MOR|MUD|NVL|ROF|RES|ROS1|ROS2|ROS3|SCO|SPT|STE|WEL|WOO|SH) (Rangos Hall|[A-Z]{0,3}[0-9]{0,4}([A-Z]{0,1}( Atrium){0,1}))|HLAS|TBA))(.*)"
...
json.dump(finals, open("finals.json", 'w'), indent=4, default=str)